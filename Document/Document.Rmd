...
calculate_rolling_median <- function(column, window_size = 200) {
  sapply(seq_along(column), function(i) {
    start <- floor((i - 1) / window_size) * window_size + 1
    end <- min(start + window_size - 1, length(column))
...
df=df%>%
  mutate(median_log_VeDBA=calculate_rolling_median(log_avg_VeDBA))

#plot(df$dt, df$median_log_VeDBA)

#Calculate rolling median of the avg VeDBA for 10 minute window
df=df%>%
